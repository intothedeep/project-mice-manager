...
>
> The line this replaces read "GENERATED ... from the live `colony_dev`", which
> stayed true-sounding after its source was gone. In one session it misled three
> separate readers into three different answers about whether 0029 had been
> applied. `schema-doc.sh` now stamps a generation date and the highest migration
> on disk, so the next regeneration overwrites this notice with facts that can be
> checked. Do not hand-edit otherwise — rerun the script.
>
