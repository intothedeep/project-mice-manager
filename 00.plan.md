...
- **FOREIGN KEYS STAY ON, even pre-production (DECIDED 2026-09-04).** The user
  wanted to insert/update/delete by hand during prototyping without constraints
  getting in the way. Resolution: keep the FKs and bypass them PER SESSION when
  hand-editing — `SET session_replication_role = replica;` disables FK and
  trigger checks for that connection only, `= origin;` restores them. Schema
  untouched, app still protected.
  WHY NOT "drop FKs and add them later": `ALTER TABLE ... ADD FOREIGN KEY` FAILS
  if a single orphan row exists, and by then it is usually impossible to tell
  which rows are wrong — the practical outcome is discarding data or abandoning
  the FK. It also matters HERE specifically: distinguishing "mate legitimately
  unresolved" from "reference broken by a typo" is the core of the import
  design, and without FKs those two look identical.
  `DEFERRABLE INITIALLY DEFERRED` remains available for the import path if
  insertion order ever becomes awkward inside one transaction.
...
- Schema refinements (architect-validated, 2026-09-04):
  ...
  - **R10 workbook-driven schema redesign (DECIDED 2026-09-04 — real-file
    ...
    **NEW TABLES:** `matings` — one row per breeding cycle:
    subject_mouse_id, nullable mate_mouse_id + mate_raw_label, cage_id,
    mated_on + is_mated_on_approx + mated_on_raw, expected_delivery_on +
    approx + raw, nullable litter_id as the OUTCOME, provenance, deleted_at;
    ...
    '260629 260817' becomes two rows. `notes` — append-only, self-referencing origin_note_id
    DEFAULT gen_random_uuid() as the LOGICAL id, subject_mouse_id, nullable
    mating_id, signal cell_signal, body, actor_id; plus `current_notes`
    DISTINCT ON view; plus reject_mutation row AND truncate triggers.
    ...
    **WHY THIS BEATS THE UUID:** a UUID `thread_id` is not the PK of any row,
    ...
    current-state read now needs DISTINCT ON via a `current_tasks` view, and
    UNIQUE constraints cannot be placed on mutable fields. The
    append-only-with-origin-id pattern now applies uniformly to `notes` and
    `tasks`; `matings` stays a normal MUTABLE soft-delete table (its state
    ...
    **TASKS vs MATINGS (user question, recorded):** tasks = WORK TO DO
    ...
    `matings.litter_id` is the outcome.
    **SERVICE-LAYER INVARIANTS (cannot be DB constraints):** mating
    ...
    duplicates); matings.litter_id must point at a litter with the same
