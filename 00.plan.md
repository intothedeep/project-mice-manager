...
- `litters.line_id` → `litters.subcolony_id`; `mouse_meta.line_id` →
  `mouse_meta.subcolony_id`.
- **A mouse belongs to the same programme as its litter**, pinned by a composite
  FK (`litters UNIQUE (id, subcolony_id)` + `mouse_meta (litter_id,
  subcolony_id)` ON UPDATE CASCADE) — the same device used for `litter_code`.
