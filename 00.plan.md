...
`colonies → subcolonies → cages → slots` are unchanged. Chain is CIRCULAR
...
- **notes REDESIGNED.** `subject_mouse_id` is now NULLABLE — it was NOT NULL,
  ...
  common note (1023 occurrences). Added `note_type` + `meta JSONB` (per-type
...
### R19 — a subcolony IS a mouse line (2026-09-05, user)

`mouse_lines` is GONE: it was the same entity as `subcolonies` under another
name. **One workbook = one colony**, and its column-B "Mouse line" values are
that colony's subcolonies:

```
colonies   MouseRoomSheet-260823-ALJ
  subcolonies   nNf1 flox;ccEGFP (85 mice) · pHRasG12V;ccEGFP (42) ·
...
spans two — so `cages.subcolony_id` is correct and the hierarchy is real, not
imposed.

- `mouse_meta.line_id` → `mouse_meta.subcolony_id`. **`litters` carries NO
  subcolony** (amended 2026-09-05, user): the programme is reachable via
  `mate_id -> mates.subcolony_id`. See R20.
- **A mouse MAY belong to a different programme than its litter** (decided
  ...
  its own litter), subcolony is not (a mouse can be reassigned).
- `subcolonies UNIQUE (colony_id, name)`: a line name is unique within its
  colony, not globally, so two workbooks can each have a 'WTs'.

NOT to be confused with `mouse_genotypes`: the subcolony is the professor's
...
VERIFIED 2026-09-05: colony → subcolony → cage chain built; a mouse claiming a
different subcolony from its litter REJECTED; a duplicate line name within one
colony REJECTED.

### R20 — the pairing carries subcolony and cage (2026-09-05, user)
> **REVERSED the same day (R21): both are derivable from the parents and were
> removed again. Read R21.**

`mates` gains `subcolony_id` and `cage_id`; `litters.subcolony_id` is REMOVED.
...
it through `mate_id -> mates.subcolony_id`. Storing it again on `litters` is the
...
`mouse_meta.subcolony_id`, so the information is not destroyed, only unreachable
from the litter row.
...
`litters.subcolony_id` confirmed gone.

### R21 — derive the pairing's programme and cage; `mice.effective_at` (2026-09-05, user)

Reverses R20. `mates` carries NEITHER `subcolony_id` NOR `cage_id`.
...
### R22 — the pairing has a lifecycle (2026-09-05, user)

When the professor orders a mating, one parent is MOVED into the other's cage to
co-house them. `mates.status` tracks where that pairing has got to:
...
**One TIMESTAMP per stage** — `cohoused_at`, `awaiting_at`, `pregnant_at`,
`delivered_at` — and **`status` is GENERATED from them** (`GENERATED ALWAYS AS
... STORED`), falling back to `pending` when all four are NULL.

Deriving it is the point: with `status` writable alongside four timestamps, a
row could claim `'pregnant'` while `delivered_at` was set. Now they CANNOT
disagree — the same reasoning as the composite FKs, applied within one row. To
advance a pairing you set a timestamp; `status` follows, and clearing a
timestamp walks it back. Writing `status` directly is rejected by Postgres.

`status` is the CURRENT state of this couple's ONGOING cycle. Past cycles are
not overwritten — each is its own `litters` row with its own dates — so the
column answers "what is happening now", never "what has ever happened".

**Every transition is logged to `audit_logs`** (`entity='mates'`, with
`before_json`/`after_json`). That is the standing who-changed-what rule and is
why no mate-status-history table is added.

**Co-housing is VERIFIED, NOT CONSTRAINED.** Once cohoused — and still at
delivery — both parents' current `mice` rows share subcolony, cage and slot.
Checking that means comparing head-of-history rows across two mice, which a
CHECK cannot express and a trigger could only reject silently; the service runs
the query and can name the parent that is in the wrong cage. The query is in
`packages/db/scenarios/flow-mating.sql`.

Note this is the third rule of this shape (with group eligibility and the
unplaced-mouse rule): cross-row conditions live in the service, single-row and
referential ones live in the schema.

VERIFIED 2026-09-05 by walking the whole flow: father moved into the mother's
cage; both parents reporting one distinct cage, slot and subcolony after
co-housing AND again at delivery; all five transitions present in `audit_logs`
with the correct actor on each; an invalid status value rejected by CHECK.
