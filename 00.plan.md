...
**Cycle dates live on the cycle.** `litters.mated_on` and its approx/raw
siblings are gone; `expected_delivery_on` moved to `mates`. `litters` holds the
OUTCOME only — `litter_code`, `seq`, `birth_date`, `pup_count`.

**`mate_status_logs` — progress is APPEND-ONLY**, one row per transition:
...
Current status is the newest row (`DISTINCT ON (mate_id) ... ORDER BY mate_id,
occurred_at DESC, id DESC`), served by `mate_status_head_idx`.

WHY A LOG AND NOT COLUMNS. A `status` column plus four stage timestamps was
built first and then replaced. The timestamps could not record WHO advanced the
...
not hold a reason. Keeping both a log and the timestamps would have recreated
the two-sources-of-truth bug that removed `mouse_attr_logs` (scenario P3), so
status is DERIVED from the log and never stored beside it.
...
rejected; both parents sharing one cage/slot/subcolony at co-housing and again
