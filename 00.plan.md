...
- **Authorization (data-driven permissions) — schema lands P1, switchover P2 (added 2026-09-13, user):**
  ...
  (`taskFlow.ts` already updated); (b) `from_status` NULL = "any source" widens (staff could pull
  a `verified` task back) — pin source or accept; (c) per-`case_type` granularity =
  YAGNI for now. See Open Questions.
