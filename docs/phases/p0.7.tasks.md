...
name of the third punch location) left — **Q43's narrower sub-question was
RESOLVED by the OWNER 2026-09-22** (*"tissue is independent each other with
toe"*: a Tissue-collection case does NOT mint a `toe` punch row). Q43-narrow was
the last gate on task **13** (the 10+12 merge); 13 is now blocked by step 7 and
task 11 only. **Q42 was never recorded as a blocker on 10, 12 or 13, and its own
text says it gates nothing on the creation path — so the step-4 GATE may now hold
nothing. NOT closed here; that is the owner's call, and Q42 itself is left
untouched.**
...
consumers) and **13 — the MOUSE-EDITING DRAWER (`[ ]`, the 10+12 MERGE): edit
mode on the drawer's Identity section PLUS the punch add/remove section, P0.7-b,
BLOCKED-BY 7 and 11 — Q43-narrow answered, no gate left).**
...
- [ ] 4. **GATE — professor (a message, not work):** put plan §5 Q39–Q45 to
      ...
      `MouseCell.pupOffsets`), **Q43-narrow RESOLVED 2026-09-22 by the OWNER**
      (*"tissue is independent each other with toe"* — no `toe` row from a
      Tissue-collection case; recorded as a RESOLVED line under Q43). Still to put
      to the professor: **Q42** only (real name of the third punch location + the
      `(toe)`-suffix conflict). Q42 was never recorded as a blocker on 10, 12 or
      13, and its own text says it gates nothing on the creation path — so this
      GATE may now hold nothing. NOT closed here; the owner decides, and Q42 is
      left untouched.
...
architect item (the ZERO-ACTIVE-PUNCHES FLOOR) is carried into 13's OPEN section;
...
- [ ] 13. **MOUSE-EDITING DRAWER — field editing + punch records in ONE task.
      ...
      **OPEN, ARCHITECT OWNS THE RULING — do NOT design it here:**
      **ZERO ACTIVE PUNCHES — still UNDECIDED** (floor vs no floor: may a mouse end
      up with zero active punches at all?). Carried from step 10. Adopting
      `untagged` did NOT pick a branch, and 8d's design deliberately did not close
      it — an ordinary fourth value changes what CREATION mints, not what REMOVAL
      may leave behind. The `untagged` adoption itself and the owner's verbatim
      words live on task **8d** (*"when babies came to us, they might not have a
      tag…"* / *"add this type into enum for punchs table"*) — read the scope
      there, not here.
      THE THREE COLLISIONS, carried over from old step 10 because whoever reads
      this next still needs them (8d answers (1) and (3); Q42 is left as-is per
      the owner, 2026-09-22): (1) `punch_location` is a CLOSED CHECK in
      `0026_punches.sql` — a fourth value is a MIGRATION (PAID by 8d scope (a));
      (2) **plan §5 Q42 is OPEN and asks precisely whether that CHECK is closed or
      the suffix set is open**, so the two belong in one answer; (3) it may
