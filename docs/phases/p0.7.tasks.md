...
MOUSE-DETAIL DRAWER, not the grid cell" ruling is SUPERSEDED with it — the owner
reopened the surface as "edit modal **or** mouse drawer", so **do not carry
"drawer" forward as decided** (step 12). **DELIBERATE GAP, not a regression:**
...
app. Whoever reads this next must not "fix" that gap by reinstating a control.
...
consumers) and 12 — the REPLACEMENT edit surface (`[ ]`, BLOCKED-BY 11 + an
architect ruling on modal-vs-drawer, on the part-by-part label editor, and on
whether it belongs to P0.7-b or P0.8).** Plus one recorded-only, no-AC item at the end (`NewTaskDialog` importing
...
- [ ] 10. Punch add/remove UI in the mouse-detail drawer (location picker,
      ...
      **THIS AC IS NOT STALE UNDER OPTION B AND DID NOT CHANGE** (recorded so it
      ...
      (ii) **OWNER DECIDED 2026-09-22 — ADOPT the `untagged` punch location; the
      system-architect is now DESIGNING it.** (This SUPERSEDES the earlier
      ...
      table."* The three collisions above are NOT dissolved by the decision —
      they are what the architect's design must answer (collision (1) means the
      adoption COSTS A MIGRATION, and collision (2) means it must be answered
      TOGETHER with Q42, which is still at the step-4 professor GATE).
      No AC here on purpose: do not invent the fix before the design lands.
      Note that (i) above — the zero-active-punches FLOOR — is STILL OPEN: the
      owner adopted `untagged` without picking floor-vs-no-floor, and the
      architect's design may or may not close it.
...
- [ ] 12. **REPLACEMENT edit surface — OPEN, the system-architect owns the
      design ruling. DO NOT DESIGN IT HERE.** Opened together with 11 so the
      owner's requirement survives the deliberate no-edit gap.
      ...
      OPEN — the architect rules, the owner picks; leave these open and
      attributed:
      (i) **modal vs drawer** — the owner named both; the earlier "MOUSE-DETAIL
      DRAWER" ruling belonged to the now-removed inline editor and is SUPERSEDED,
      so it is NOT inherited as decided;
      (ii) the **shape of the part-by-part label editor** (which parts are
      directly editable vs derived — e.g. the ear-punch suffix is derived from
      punch rows, which is step 10's surface);
      (iii) **which phase this belongs to** — P0.7-b, or P0.8 "Visual cage
      management" (P0-c, MVP v0.3). It sits in this file until the architect
      rules; moving it is a single edit plus its `TASKS.md` stub.
      COORDINATION POINT (recorded, not designed): step **10** also lands an edit
      surface in the mouse-detail drawer (punch add/remove). Whoever designs 12
      must look at 10 first, or the drawer grows two unrelated edit idioms.
      BLOCKED-BY: **11** (the removal lands first — owner's explicit order) **and
      the architect ruling on (i)-(iii)**.
      NO FULL AC YET — writing one before the ruling would invent the design. But
      the later task MUST carry these two falsifiable criteria, which are not
      design choices but the 9a/9b closure:
      - no FREE-TEXT whole-label input exists anywhere, and no code path infers
        `sex` from a typed label (grep: nothing writes `sex` from a parsed
        string);
      - **no `mouseLabel:` KEY DECLARATION survives under `packages/types/src`**
        — 9d's RESTATED form. Do NOT write the bare-identifier grep
        ("`mouseLabel` returns 0"): it returns 2 legitimate hits (the surviving
        `lib/mouseLabel.ts` module name, and prose) and is unmeetable. See the
        FIFTH OCCURRENCE in the PATTERN block.
