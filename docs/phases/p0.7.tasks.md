...
> forgotten.** This file is ~1140 lines, past `rules/docs.md` §1's 400 HARD line,
> and a further split WAITS until P0.7-b closes. Splitting mid-flight would move
> files out from under in-progress tasks (6, 6a, 11 are live right now), and §2
...
build top-down. **DONE: steps 1, 2, 3, 5, 8, 8b, 8c, 9a, 9b, 9c, 9d.** Step 4 is a
MESSAGE, not work: it stays open as the professor GATE with only **Q42** (real
name of the third punch location) and **Q43's narrower sub-question** (does a
Tissue-collection case ALSO mint a `toe` punch row?) left; Q43-narrow gates step
10 only, nothing else.
...
has now RULED 2026-09-22: the surface IS the DRAWER, task 12 is P0.7-b, and it is
SEQUENCED AFTER step 10.** The "do not carry 'drawer' forward as decided" note
written earlier today is therefore SPENT; 12 carries the full design.
...
REBASES onto 6a, it does not restart. Step 7 BLOCKED-BY 6; step 10 BLOCKED-BY
**6a + 7 + Q43-narrow**. Also open: **9e — DELETE the dead code (owner DECIDED
...
**6a and** step 6 (all three touch `packages/types/src/index.ts`). **NEW 2026-09-22: 11 — REMOVE inline cell
editing (`[~]`, developer dispatched; deletes `editable-cell.tsx` + its three
consumers) and 12 — the REPLACEMENT edit surface (`[ ]`, DESIGN RULED: edit mode
on the drawer's Identity section, P0.7-b, BLOCKED-BY 11 and SEQUENCED AFTER 10).
...
- [ ] 6a. **Punch history contract — NEW 2026-09-22, OWNER DECIDED option B.**
      ...
      **It belongs HERE, not in step 6** — it is the mechanical consequence of
      ...
      BLOCKS: 6 (and therefore 7 and 10). BLOCKED-BY: nothing.
...
- [ ] 8d. **`untagged` punch location — OWNER DECIDED 2026-09-22 (ADOPT + the
      ...
      unique index, no nullability change. `effective_at` = when the observation
      ...
      **VERIFY THE CONSTRAINT NAME FIRST, do not assume it** — `SCHEMA.md` lists
      ...
      `location === 'ear'`).
      STILL OPEN, not decided here: step 10's zero-active-punches FLOOR (may a
      mouse reach zero ACTIVE punches at all?).
      ...
      (6) `buildMouseCell` contains NO location literal at all.
...
- [x] Inline `sex` editor — **SUPERSEDED 2026-09-22, NOT SHIPPED AS A FEATURE.**
      ...
      **KEEP — do NOT delete these along with the control (task 11 ring-fences
      them, task 12 is their consumer):** the STALENESS FIX (the drawer looks
      ...
      "edit modal **or** mouse drawer". Task 12 must NOT inherit "drawer" as
      decided. UNBLOCKED 2026-09-22: its blocker 9b is DONE
...
- [ ] 10. Punch add/remove UI in the mouse-detail drawer (location picker,
      effective date, note; remove = soft-delete via step 7). BLOCKED-BY: **6a, 7
      and Q43's narrower sub-question** (does a Tissue-collection case ALSO mint a
      `toe` punch row? — plan §5 Q43, still at the step-4 GATE; it decides whether
      this UI shows a second mint site)
      ...
      drawer's history, hidden from the active list — **TIGHTENED 2026-09-22:
      surfaced via the punch-history selector `usePunchLog` (step 7), NOT via
      `MouseCell.punches`**); no punch control exists outside the drawer.
      **THIS AC IS NOT STALE UNDER OPTION B AND DID NOT CHANGE** (recorded so it
      is not re-litigated): it never named `MouseCell.punches` as the source, so
      B satisfies it as written; the 2026-09-22 clause above only forecloses the
      re-reading that caused step 6's schema flip.
      OPEN, ARCHITECT OWNS THE RULING — do NOT design it here, and do NOT treat
      task 8c as defective on account of it. Two linked questions:
      (i) **ZERO ACTIVE PUNCHES — still UNDECIDED** (floor vs no floor: may a
      mouse end up with zero active punches at all?). The owner answered it with
      the proposal below rather than picking either branch, so it stays OPEN.
      (ii) **OWNER DECIDED 2026-09-22 — ADOPT the `untagged` punch location.
      CARRIED BY NEW TASK 8d — read the scope there, not here:** `untagged` is an
      ORDINARY fourth value of `punch_location` (migration `0028`), the add dialog
      DEFAULTS to it, and it renders nothing. Collision (1) is PAID (8d scope
      (a)); collision (3) resolves by the `'toe'` literal simply leaving
      `buildMouseCell`. Q42 is UNTOUCHED and stays exactly as it is.
      **(i) above is STILL OPEN** — the floor was not decided by any of this.
      (This bullet SUPERSEDES the earlier
      PROPOSAL framing. The architect had recommended AGAINST adoption; the owner
      overruled that. The adoption is the owner's; the HOW is the architect's,
      and is not designed here.) The words,
      kept verbatim: *"when babies came to us, they
      might not have a tag, so we can create a punch with new enum type:
      untagged? so we can take care of 0 punch tag case."* THREE COLLISIONS,
      recorded because whoever reads this next needs them: (1) `punch_location`
      is a CLOSED CHECK in `0026_punches.sql` — a fourth value is a MIGRATION;
      (2) **plan §5 Q42 is OPEN and asks precisely whether that CHECK is closed
      or the suffix set is open**, so this proposal and Q42 must be answered
      together; (3) it may contradict task **8c, which SHIPPED today** —
      `buildMouseCell`/`addMouse` mints `location: 'toe'` at creation, which
      asserts a physical mark that an untagged pup does not have. **8c is NOT
      marked defective on account of this** — it shipped against the contract as
      it stood.
      The owner's adoption words, verbatim: *"add this type into enum for punchs
      table."* Collisions (1) and (3) are answered by task 8d; Q42 is left as-is
      per the owner (2026-09-22).
      RESOLVED 2026-09-22: the work landed as **task 8d**; this bullet keeps no
      AC of its own.
      Note that (i) above — the zero-active-punches FLOOR — is **STILL OPEN**:
      the owner adopted `untagged` without picking floor-vs-no-floor, and 8d's
      design deliberately did NOT close it (an ordinary fourth value changes what
      creation mints, not what removal may leave behind).
- [~] 11. **REMOVE inline cell editing — OWNER DECIDED 2026-09-22; IN PROGRESS,
      developer dispatched.** Owner verbatim, so attribution sits on the task and
      ...
      **ORDER IS THE OWNER'S: removal FIRST, replacement (task 12) LATER.** The
      ...
      **RING-FENCED, DO NOT DELETE** (these are task 12's consumers, and on the
      ...
      leaving. Reword them — the live lookup STAYS and still needs a WHY (task 12
      will write through it again).
      BLOCKED-BY: nothing. BLOCKS: 12.
...
- [ ] 12. **REPLACEMENT edit surface — DESIGN RULED 2026-09-22 (was OPEN; the
      architect has now ruled (i), (ii) and (iii)).** Opened together with 11 so
      the owner's requirement survives the deliberate no-edit gap.
      OWNER'S CORE REQUIREMENT, verbatim: *"we use a edit modal or mouse drawer
      to update a mouse data"* / *"for mouse label we will show all part by part
      so we can edit each."*
      WHAT THAT MEANS HERE (recorded, not designed): the label is shown **PART BY
      PART** — sex, pup number, `+offset`, litter code, ear-punch suffix — so
      each PART is editable. That is exactly the CLOSURE of **9a** (no free-text
      label edit, no `sex` back-inference from a typed string) and **9b** (no
      stored label; the label is a read-time projection). The part editor must
      not reintroduce either.
      RULED 2026-09-22 — these three were open; they are now decided:
      ...
      modal would have to re-solve from scratch (task 11 ring-fences it for
      exactly this consumer). *This REPLACES the "do not carry 'drawer' forward
      as decided" note written earlier today — the owner reopened the surface as
      "modal or drawer" and the architect has now picked drawer.*
      ...
      transfers / drag-and-drop — none is a field-edit surface, and parking 12
      there would leave the app with NO edit path across a whole phase.
      ...
      - punches (`e…`) — **RECORDS.** A read-only PROJECTION ("2 active ear
        punches → `ee`") linking to the drawer's punch section (step 10). **NO
        second add/remove UI** — one mutation path.
      ...
      a second builder.**
      BLOCKED-BY: **11** (the removal lands first — owner's explicit order).
      **SEQUENCED AFTER step 10** — an ordering with teeth: 12's punch row is a
      read-only projection POINTING AT step 10's section, so building 12 first
      leaves it pointing at nothing. (This replaces the old "look at 10 first or
      the drawer grows two edit idioms" coordination note — same concern, now an
      order.)
      AC — written as **DATA-FLOW DIRECTION**: composed strings flow OUT to
      render, never IN to a mutation.
