...
(`lib/punchMutations.ts` — its only sequencing condition, 8c landing
`Counters.nextPunchId`, is SATISFIED / `4d5870e`) **and the inline sex editor**
(its design question is DECIDED by the owner 2026-09-22 — the control goes in the
MOUSE-DETAIL DRAWER, not the grid cell). Step 7 BLOCKED-BY 6; step 10 BLOCKED-BY
7 + Q43-narrow. Also open: **9e — DELETE the dead code (owner DECIDED
...
step 6. Plus one recorded-only, no-AC item at the end (`NewTaskDialog` importing
...
- [~] 6. **IN PROGRESS — developer dispatched 2026-09-22.**
      ...
      the reason. Remove = set deletedAt, never splice.
...
DRIFT is a DIFFERENT CLASS, ruled here so it is not re-argued as a sixth
occurrence (2026-09-22): the three stale references `afa8c43` fixed — the `plan §7`
...
- [ ] 9e. **DELETE the dead code — OWNER DECIDED 2026-09-22** (the owner reviewed
      ...
      SEQUENCED AFTER STEP 6 (an ordering, not a blocker): this touches
      `packages/types/src/index.ts` and step 6 is in flight and may touch
      `packages/types` too.
...
- [ ] 10. Punch add/remove UI in the mouse-detail drawer (location picker,
      effective date, note; remove = soft-delete via step 7). BLOCKED-BY: **7 and
      Q43's narrower sub-question** (does a Tissue-collection case ALSO mint a
