...
Remaining: 6–7 BLOCKED-BY the GATE; 8b and the sex editor unblocked NOW (their
only blocker, 9b, closed 2026-09-22 / `8ae117e`); 10 BLOCKED-BY 7; NEW 8c (the
toe punch step 8's AC claimed and never implemented — its dependency on 7 is an
OPEN architect ruling); NEW 9c (`extractLitterCode` has zero consumers after 9b);
plus a recorded-only `NewTaskDialog`/`SEED_COLONY` item at the end (architect).
Mock-era, no server writes.**
...
- [ ] 8b. Split `lib/colonyMutations.ts` — carries step 8's unmet criterion (i),
      ...
      gone.
      INTERACTION WITH 8c (recorded, NOT ruled here): 8c adds the toe-punch mint
      to `addMouse`, which lives in this same file, so 8b and 8c touch the same
      neighbourhood. Whether 8b splits before or after 8c is UNDECIDED — the
      system-architect owns that ordering, together with 8c's dependency on 7.
...
- [ ] 8c. Mint the implicit `toe` punch in `addMouse` — carries step 8's unmet
      ...
      BLOCKED-BY: **OPEN — system-architect owns this ruling.** Minting needs a
      punch id; `nextPunchId` is step 7's deliverable, step 7 is BLOCKED-BY 6,
      and 6 is BLOCKED-BY the professor GATE. So 8c either WAITS on 7 or
      PRE-EMPTS it with an interim id source. Not decided here. The 8b↔8c
      ordering (same file) rides the same ruling.
      ...
      AC (direct field inspection, §19): the colony returned by
      `addMouse(cageId, slotId, mouse)` has, for the new mouse,
      `punches.length === 1` with `location === 'toe'` and a non-null
      `effectiveAt`; two consecutive `addMouse` calls give the two toe punches
      DISTINCT ids; grep `['"]toe['"]` across `apps/colony_client_web` SOURCE
      (exclude the `.next/` build output) hits exactly TWO files — the fixture
      `apis/getColonyGrid.mock.api.ts` (21 occurrences today) and the ONE new
      minting site (VERIFIED 2026-09-22: today the fixture is the only source
      file with a `'toe'` literal, so this AC is falsifiable, not vacuous); grep
      `Toe-punch minting on creation is step 8's own AC` repo-wide returns 0
      (the divergence comment is deleted — phrase, not line number, because 8b
      may move it); check-types + lint 0 errors.
...
- [ ] 9c. `extractLitterCode` has ZERO consumers — carries the repo quality
      gate (`rules/development/code.md`, "no dead code / unused exports") that
      9b's close left unmet (`rules/docs.md` §7). 9b removed its last two
      consumers (`mockColonyStore` and `colonySeed` now read `m.litterCode`
      directly), and 9b's AC EXPLICITLY FORBADE deleting it: steps 3 and 5 both
      require the legacy-form parser (`F10+1earAYY`, hand-written cage cards and
      the workbook) to keep existing. So the symbol must survive AND must gain a
      live consumer.
      **An `x_` rename is NOT a soft delete here.** Both apps' `tsconfig.json`
      carry `exclude: ["**/x_*"]`, so renaming the module out of the way removes
      it from the BUILD — that is a deletion wearing a soft-delete name, and it
      would silently drop the legacy parser steps 3 and 5 depend on.
      WHO CONSUMES IT: **OPEN — the system-architect is naming the intended
      consumer** (the externally-sourced-string boundary: import path, cage-card
      entry, workbook paste). Not decided here.
      AC: repo-wide grep `extractLitterCode` yields 1 definition + ≥1 LIVE
      consumer, and every such consumer parses an EXTERNALLY-sourced string
      (import / hand-entered / cage card), never a `MouseCell`; OR this task
      carries the architect's written ruling that it is genuinely dead, naming
      the removal path — which may NOT be an `x_` rename. `parseLitterCode` (the
      base-26 codec) is unrelated and must not be touched. check-types + lint
      0 errors.
