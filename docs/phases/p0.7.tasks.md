...
- [~] 6. **STILL `[~]` 2026-09-22 (landed `96f5a8d`) — BLOCKED-BY: 8g** (6a is CLOSED; the old
      `BLOCKED-BY: 6a` is satisfied). **ONE CRITERION IS UNMET, so this task does
      NOT tick:** the AC clause "every other mouse is reference-equal" (see the
      AC below). It HELD at step 6's own commit `96f5a8d`, where `addPunch`
      mapped `m.metaId === input.metaId ? { ...m, punches } : m`; task 8e
      (`20e020d`) replaced that with `projectPunches`, which spreads EVERY line,
      cage, slot and mouse unconditionally. **8e did not decide this** — 8e's own
      shape text says `grid.punches` "becomes a PROJECTION", never "rebuild every
      object". The clause therefore stands AS WRITTEN and NEW task **8g** carries
      it. **It is NOT RETIRED and NOT annotated as retired:** RETIRED is the
      device for a CLOSED task whose shipped criterion a later task contradicted
      (8b→11, 8d→8f). This task is OPEN, so the criterion is simply unmet and
      keeps its checkbox via 8g.
      ...
      **[UNMET as of 2026-09-22 — this clause is why 6 is still `[~]`. Held at
      `96f5a8d`, broken by 8e (`20e020d`, `projectPunches` spreads every object).
      CARRIED BY 8g. Not retired: this task is open. Text left exactly as
      written — 8g closes it, it is not reworded away]**, AND one row
...
- [ ] 8g. **Restore step 6's "every other mouse is reference-equal" —
      NEW 2026-09-22.** Step 6's AC says an `addPunch` must return a colony in
      ...
      **AC, not an assumption** (AC (4) below).
      ...
      (6) check-types + lint 0 errors across the monorepo.
- [ ] 8h. **`punchId` uniqueness guard at `commit()` — NEW 2026-09-22.**
      ...
      **THE GUARD HAS TWO HALVES WITH DIFFERENT LIFESPANS — record it, because
      ...
      or a test runner):
      (1) `mintPunch` is the only place that re-derives `{ grid, punches }` —
