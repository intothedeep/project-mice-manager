...
☐ `ccEGFP(hmo)`를 `ccEGFP`와 **다른 마커**로 기록했습니다. "ccEGFP인데 동형접합"이
  아니라요. **랩에서 실제로 그렇게 구분하십니까?**
...
☐ 9   Add mouse 의 유전자 배지
      → PlpCre · Nf1 · Ai14 · ccEGFP · ccEGFP(hmo) · WT  (이 순서)
...
⚠️ **그 방향은 지금 만든 것과 반대입니다.** 지금은 `(hmo)`가 **코드의 일부**(카탈로그
행)인데, 스케치는 **조인 행의 표시**로 만듭니다. 그러면 `ccEGFP(hmo)` 카탈로그 행이
없어져야 합니다. 나중에 여실 때 "컬럼 추가"로 끝나지 않고 **한쪽을 되감아야** 합니다.
