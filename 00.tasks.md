...
- [ ] AMEND 0005: `tasks` APPEND-ONLY (user directive, plan §4 R10) — add
      ...
      `current_tasks` DISTINCT ON view. COST accepted (recorded R10):
      current-state reads go through current_tasks; no UNIQUE constraints on
      mutable fields.
      AC: UPDATE/DELETE/TRUNCATE on tasks rejected by trigger; two rows
      sharing one origin_task_id → current_tasks returns exactly the latest
...
- [ ] NEW 0007_breeding_and_records.sql: `matings` (one row per breeding
      ...
      expected_delivery_on + approx + raw, nullable litter_id = the OUTCOME,
      ...
      settled note (Q29) — body, actor_id) + `current_notes` DISTINCT ON
      ...
      origin_note_id → current_notes returns exactly the latest; a two-date
...
duplicates); matings.litter_id must point at a litter with the same parent
...
- [ ] Lifecycle tables: matings, lifecycle_events (+ sample_type / lifecycle_event
      ...
      note is SUPERSEDED (matings and litters coexist: matings.litter_id is
